...
void proc_ln() {
  ...
  // EXIT ROUTINE (terminal code path) : 
  if( check_end(&ln) == -1) {   
  ...
  if( strcmp(ln,"\n")!=0 ) {   // do nothing at newline 
    // MOMENT OF TRUTH - NAILING ANY COMMAND LINE TEXT : 
...
void get_cmds(char* cmds[] , int * cmds_num, char * cl_rem){  
  const char * delim_amp = "&";
  const char * delim_space = " ";
  char * cmd = NULL;

  while ( (cmd = strsep(&cl_rem, delim_amp)) != NULL ) { // cmd = "fefsef fesfe " & " efes fesfe "
    printf("cmd : %s\n" , cmd);
    //*cmds_num++; 
    strsep(&cl_rem, delim_space);
  }
